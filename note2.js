...
function renderStudentNotes() {
  ...
  const filteredNotes = lessonNotes.filter(
    note => note.class === currentUser.class
  );

  if (filteredNotes.length === 0) {
    studentNotes.innerHTML = "<p>No lesson notes available for your class yet.</p>";
    return;
  }
...
function renderManageNotes() {
  ...
  let filteredNotes = lessonNotes;
  if (currentUser.role === "teacher") {
    filteredNotes = lessonNotes.filter(note =>
      currentUser.subjects.includes(note.subject)
    );
  }

  if (filteredNotes.length === 0) {
    manageNotes.innerHTML = "<p>No lesson notes available for your subjects.</p>";
    return;
  }
...
// ----------------------------
const usernameDatalist = document.createElement("datalist");
usernameDatalist.id = "usernames-list";
document.body.appendChild(usernameDatalist);

usernameInput.setAttribute("list", "usernames-list");
